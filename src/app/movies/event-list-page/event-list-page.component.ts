...
export class EventListPageComponent {
  ...
  showallevent = true;
  catId: any;
  pageindexforsize: number | undefined;
  ...
  ngOnInit() {
    ...
    // this.mulTime = localStorage.getItem('Time');
    // this.venues = localStorage.getItem('Venues');
    this.mainShowDates = this.Dates ? JSON.parse(this.Dates) : [];
    // this.mainShowTime = this.mulTime ? JSON.parse(this.mulTime) : [];
    // this.mainShowVenues = this.venues ? JSON.parse(this.venues) : [];

    // console.log('datetime',this.mainShowDate,this.mainShowTime)
    ...
    // this.route.queryParams.subscribe((params) => {
    //   this.catId = Number(params['catId']);
    // })
    this.isMobile = this.apiService.isMobileDevice();
    ...
    // this.getinterests();
    // this.getreviewa();
    const result = this.getMinMaxFromStorage();

    this.isSingleDate = result.isSingle;
    this.minShowDate = result.minDate;
    this.maxShowDate = result.maxDate;
    this.singleShowDate = result.date;
    this.singletime = result.time;
    // console.log("result data")
    // console.log("datae", this.singleShowDate)
    // console.log("Tinme", this.singletime)
    this.getPlayDetails();
  }
  ...
  getPlayDetails() {
    ...
    this.searchLoading = true;
    this.apiService
      .getMainPlayDetailsPage(this.playId, this.selectedCityId, this.memberId, this.isGuest)
      .subscribe(
        (data: any) => {
          if (data['code'] === 200 && data['data'][0]) {
            ...
            // console.log("play details", this.PlaysData)
            ...
            if (
              ...
              // sessionStorage.setItem('cleanedTerms', JSON.stringify(this.PlaysData.TERMS_CONDITIONS));
            } else {
              this.cleanedTerms = [];
            ...
            this.TAGS_NAMESList = this.PlaysData?.TAGS_NAMES
              ? this.PlaysData.TAGS_NAMES.split(',').map((tag: any) => tag.trim())
              : [];
  ...
  onScroll(): void {
    ...
    if (sentinel && !this.isLoadingMore && !this.isLoading && this.hasMoreEvents) {
  ...
  loadMore(): void {
    if (this.isLoadingMore || this.isLoading || !this.hasMoreEvents) return;
    this.isLoadingMore = true;
    this.page++;
    this.getRecommendedPlays(true);
  }
  ...
  // pageSize = 12;
  page = 1
  getRecommendedPlays(append = false) {
    const filter = {
      MAIN_EVENT_ID: this.playId,
    };
    const sortKey = "IS_PROMOTED"
    const sortValue = "DESC"
    const excludeEventIds = this.playId;

    this.isLoading = true;

    this.apiService.getRecommendedDataList(
      this.page,
      this.pageSize,
      filter,
      this.selectedCityId,
      ...
      (res: any) => {
        ...
        this.hasMoreEvents = newData.length >= this.pageSize;
        ...
        if (this.selectedTags.length > 0) {
          ...
          this.listofallcategorydata = this.originalList;
        }

        if (!append) {
          this.mulTime = []
          this.venues = newData.flatMap((event: any) => {
            const venues: string[] = [];
            if (event.schedules?.length) {
              const scheduleVenues = event.schedules
                .map((s: any) => s.VENUE_NAME)
                .filter((v: string) => v);
              venues.push(...scheduleVenues);
            }
            const multipleDates = event.schedules
              .map((s: any) => s.SHOW_TIME)
              .filter((v: string) => v);
            this.mulTime.push(...multipleDates);
            return venues;
          });
          this.venues = [...new Set(this.venues)];
          const scheduleVenues = newData
            .map((s: any) => s.startDate)
            .filter((v: string) => v);
          this.mainShowDates = [...scheduleVenues];
          this.isSingleDate = this.mainShowDates.length === 1;
        }
      },
      (err) => {
        this.isLoading = false;
  ...
  getEventRouterLink(section: any, event: any): any[] {
    return ['/explore', this.selectedCity, event.CATEGORY_NAME, event.EVENT_SLUG, event._id];
  ...
  evaluateBookingButton(): void {
    ...
    let earliestValidEarlyAccessDate = false;

    for (const schedule of schedules) {
      var earlyAccessDate: any = null
      if (schedule?.EARLY_ACCESS_DATE != null)
        earlyAccessDate = this.parseCustomDate(schedule?.EARLY_ACCESS_DATE);
      // const earlyAccessDate = schedule.EARLY_ACCESS_DATE ? new Date(schedule.EARLY_ACCESS_DATE) : null;
      ...
      // 1. Check if normal booking window is open
      if (currentTime >= showStartDate && currentTime <= showEndDate) {
        earliestValidEarlyAccessDate = true;

      }

      // 2. Check early access window only if user is subscribed
      if (
        this.isearlyaccess &&
        earlyAccessDate &&
        currentTime >= earlyAccessDate &&
        currentTime <= showEndDate
      ) {
    ...
    // 3. If any valid early access date exists for subscribed users
    if (earliestValidEarlyAccessDate == true) {
      this.showBookingButton = true;
      //consolelog(this.showBookingButton, "his.showBookingButton1")

      return;
    }
    //consolelog(this.showBookingButton, "his.showBookingButton")
    // 4. Otherwise hide the button
  ...
  setMetaTags(event: any) {
    if (event.eventMetaInfo != undefined && event.eventMetaInfo != null && event.eventMetaInfo.length > 0) {
  ...
  getMinMaxFromStorage(): {
    ...
    if (!this.mainShowDates || this.mainShowDates.length === 0) {
      return { isSingle: false, date: '', time: '', minDate: '', maxDate: '' };
    }

    const formatDate = (date: Date) => date.toISOString().split('T')[0];

    const dates = this.mainShowDates.map((d: string) => new Date(d));
    ...
    if (dates.length === 1) {
      return {
        ...
        time: this.mainShowTime && this.mainShowTime.length
          ? this.mainShowTime[0]
          : '',
        minDate: '',
        maxDate: '',
    ...
    // 🔹 Multiple dates
    const minDate = new Date(Math.min(...dates.map((d: { getTime: () => any; }) => d.getTime())));
  ...
  onTagSelect() {
    ...
    this.listofallcategorydata = this.originalList.filter((item: any) => {

      if (!item.TAGS_NAMES) return false;

      const tagArray = item.TAGS_NAMES
        .split(',')
        .map((t: string) => t.trim());

      // OR condition
      return this.selectedTags.some(tag => tagArray.includes(tag));
    });
  }
  clearTags() {
    this.selectedTags = [];
  ...
  gotoBookNow() {
    ...
    // if (this.PlaysData.eventScheduleData.length == 1 && this.PlaysData.eventScheduleData[0]['IS_QUEUE_ENABLED'] == true) {
  ...
  checkArrowVisibility() {
    ...
    const totalItems = this.listofallcategorydata?.length || 0;
