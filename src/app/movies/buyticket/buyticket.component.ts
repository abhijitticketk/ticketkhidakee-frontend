...
export class BuyticketComponent {
  ...
  getScheduledShowsByTheater(movieId: any) {
    ...
    this.apiService.getScheduledShowsByTheater(0, 0, '', 'asc', '', movieId, this.selectedCityId).subscribe(
      (data: any) => {
        if (data['status'] === 200) {
          this.theatres = data['data']; // ✅ assign directly
          
          // this.theatres = [/* your data here */];

          this.screenTypesSet = new Set<string>();
          this.LangaugeData = new Set<string>();

          this.theatres.forEach(theater => {
            theater.MOVIE_SHOWS.forEach((movie: any) => {
              movie.SCREEN_TYPES.forEach((screen: any) => {
                this.screenTypesSet.add(screen.SCREEN_TYPE);

                screen.LANGUAGES.forEach((lang: any) => {
                  this.LangaugeData.add(lang.LANGUAGE);
                });
              });
            });
          });

          const screenTypes: string[] = Array.from(this.screenTypesSet);
          const languages: string[] = Array.from(this.LangaugeData);
